...
export const unwrapToken = (
  ...
  if (
    ...
    aaveWrappedMap[aaveChain] != null && false
  ) {
    ...
    if (Object.keys(aaveWrappedMap[aaveChain]).includes(lowercase)) {
      ...
      ]; //serve per restituire il riferimento al token di wrapper. In questo momento è disattivato. quindi il costrutto if non viene mai eseguito
    } else {
      return lowercase;
    }
  } else {
