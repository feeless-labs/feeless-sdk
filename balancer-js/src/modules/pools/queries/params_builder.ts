...
export class ParamsBuilder implements PoolQueries.ParamsBuilder {
  ...
  buildQueryJoinExactIn({
    ...
    const userData = (this.encoder as PoolQueries.Encoder).joinExactTokensInForBPTOut(
      maxInWithoutBpt,
      minimumBPT
    );
  ...
  buildQueryJoinExactOut({
    ...
    const userData = (this.encoder as PoolQueries.Encoder).joinTokenInForExactBPTOut(bptOut, tokenIndex);
  ...
  buildQueryExitToSingleToken({
    ...
    const userData = (this.encoder as PoolQueries.Encoder).exitExactBPTInForOneTokenOut(
      bptIn,
      tokenIndex
    );
  ...
  buildQueryExitProportionally({
    ...
    // Verifica se la funzione exitExactBPTInForTokensOut è definita
    const encoder = this.encoder as PoolQueries.Encoder;
    if (typeof encoder.exitExactBPTInForTokensOut !== 'function') {
      throw new Error('Proportional exit not implemented');
    }

    // Genera userData utilizzando la funzione exitExactBPTInForTokensOut
    const userData = encoder.exitExactBPTInForTokensOut(bptIn);

    // Prepara i parametri per la query di uscita proporzionale
    const params: PoolQueries.queryExitParams = [
      this.pool.id,
      AddressZero, // Il sender è ignorato sulla query join, quindi può essere l'indirizzo zero
      AddressZero, // Come il sender
      {
        assets: this.pool.tokensList,
        minAmountsOut,
        userData,
  ...
  buildQueryExitExactOut({
    ...
    const userData = (this.encoder as PoolQueries.Encoder).exitBPTInForExactTokensOut(
